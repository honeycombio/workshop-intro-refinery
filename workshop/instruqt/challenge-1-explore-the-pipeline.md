...
Your sandbox is already running a full telemetry pipeline: two load generators (`loadgen1`, `loadgen2`) sending synthetic traffic through an OpenTelemetry Collector, into Honeycomb Refinery, and on to Honeycomb. You won't build any of this yourself. Your job is to understand what's already running before you start tuning it.
...
If you want a fresh burst of traffic running right now, use `./run` to restart just the load generators.
...
4. Honeycomb generates an API key for the new environment automatically. Change your environment to `refinery-workshop` using the navigation menu.
5. Select `Home` from the navigation menu. Copy the API key.
...
1. Select the [button label="Honeycomb"](tab-2) tab.
2. Select Query from the navigation menu.
3. Set the time range to the last 10 minutes.
4. Run a query:
   - **WHERE:** `app.function exists`
   - **GROUP BY:** `app.function`, `app.endpoint`
5. You should see three distinct `app.function` values, and many more `app.endpoint` values.
6. Now remove `app.endpoint` from **GROUP BY** and rerun. This simplified view is what you'll come back to in later challenges to see how your rule changes affect the data.
