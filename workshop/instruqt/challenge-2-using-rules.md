...
2. In the Query Builder's **GROUP BY**, create a new calculated field named `if 500 true`:
```
IF(
  EXISTS(
...
4. **GROUP BY:** the `if 500 true` calculated field you just created
...
2. Change **GROUP BY** to `meta.refinery.reason` and remove `HEATMAP(duration_ms)` from **VISUALIZE** (keep `COUNT`).
...
## Anomaly detection based on rule matches

1. Hover over and select a point on the line representing traces with errors, then select **Detect Anomalies**.
...
This helps identify which rules are capturing which anomalies, and whether specific error types or latencies are linked to certain rules.
...
3. Change **GROUP BY** to `app.function`.
